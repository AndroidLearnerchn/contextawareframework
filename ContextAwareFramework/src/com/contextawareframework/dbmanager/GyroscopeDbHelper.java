...
public class  GyroscopeDbHelper{
	...
	public Gyrometer createGyrometerRowData(long timestamp,Float x, Float y, Float z){
		...
		if(enableDebugging)
		{
			Log.d(TAG,"createGyrometerRowData Method");
		}
		return newRow;
	}
	...
	public void deleteGyrometerRowData(Gyrometer gyrometerRow) {
		try{
			...
			if(enableDebugging)
			{
				Log.d(TAG,"deleteGyrometerRowData Method");
			}
		}
		catch(SQLException e)
		{
	...
	/**
	 * Method to initialize a Gyrometer POJO object
	 */
	private Gyrometer cursorToGyrometerRow(Cursor cursor) {
		...
		if(enableDebugging)
		{
			Log.d(TAG,"cursorToGyrometerRow Method");
		}
		return gyroRow;
	}
} 
